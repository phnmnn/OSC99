...
boolean osc2Toggle = true; //The logic on this has to be flipped for it to show up right on the UI, I dont understand why

int controls = 1;
String current_control = "OSC1";

boolean osc1Toggle = false; //The logic on this has to be flipped for it to show up right on the UI, I dont understand why
...
void mousePressed() {
  ...
  */
  ...
  //if(dist(mouseX, mouseY, width/2, 30)<12 ){
  //      println("load");
  //    }
...
public void savePatch(){
  ...
  String[] parameter = { "Theta", "Amplitude", "AmpAndrei", "Period", "Osc 1 Speed", "Filler Lines", "Mod Amt", "Mod Direction", "Line Amount", "Wave Hue", "New Hue", "Background Hue",
                           "Hue Change", "Wave Hue LFO", "Direction", "Background Value", "LFO Speed", "Sine", "SquareWidth", "Blend Type", "Type", "OSC 2 Toggle", "Controls", "Osc 1 Toggle", 
                         ...
                         "Filler Lines 2", "Mod Amount 2", "Mod Direction 2", "Line Amount 2", "Wave Hue 2", "New Hue 2", "Background Hue 2", "Hue Change", "Wave Hue LFO 2", "Direction 2", 
                          "Background Value 2", "LFO Speed 2", "Sine 2", "Square Width 2", "OSC 2 Opacity", "Band Spacing 2", "OSC 2 Direction"};
  String[] cur_values   = { str(theta), str(amplitude), str(ampAndrei), str(period), str(osc_speed), str(filler_lines), str(modAmt), str(modDir), str(line_amt), str(wave_hue), str(new_hue), str(bgr_hue),
                            str(hue_change), str(wavehueLFO), str(dir), str(bgr_val), str(lfo_speed), str(sine), str(squareWidth), str(blendType), str(type), str(osc2Toggle), str(controls), str(osc1Toggle), 
                            ...
                            str(filler_lines2), str(modAmt2), str(modDir2), str(line_amt2), str(wave_hue2), str(new_hue2), str(bgr_hue2), str(hue_change2), str(wavehueLFO2), str(dir2), str(bgr_val2), str(lfo_speed2), 
                            str(sine2), str(squareWidth2), str(osc2_opacity), str(band_spacing2), str(oscDir2)};
...
public void loadValues(JSONArray States, String fileTitle){
      ...
      //JSONObject para = States.getJSONObject(0);
      //println(para);
      //println(para.getString("cur_value")); 
      println(States.size());
      ...
      osc_speed = float(paraValues[4]);
       ...
       osc2Toggle = boolean(paraValues[21]);
      
      
       controls = int(paraValues[22]);
       
      
       osc1Toggle = boolean(paraValues[23]);
...
public void randomize(){
  ...
  //have this to make sure both osc never end up off bc thats boring
   if ((osc1Toggle == true) && (osc2Toggle == true)){
     osc1Toggle = false;
   }
